%% Igor Pivnyk — Base CV
%% Compile with: pdflatex igor_pivnyk_cv_base_updated.tex
%% GUARD: This CV MUST fit on a single page. Keep it to one balanced page —
%% if edits cause a spill, tighten spacing/margins (or drop the \cvbasefont size
%% to 9pt) before adding a second page.

\documentclass[10pt,a4paper]{article}

% ---- Core packages (all in texlive-latex-recommended + texlive-fonts-recommended) ----
...
% ---- Real base text size. article honors only 10/11/12pt as the class option,
%      so the true body size is pinned here and applied at begin-document. The
%      one-page shrink ladder retunes the body by re-issuing \cvbasefont. ----
...
% ---- Section titles (replaces titlesec). \cvsecbefore/\cvsecafter are tunable
%      lengths so the one-page shrink ladder can retighten section spacing. ----
...
% Redefine \section to \cvsection while still tolerating the star form
% (\section*{...}) and the optional short-title arg (\section[Short]{Long}) that
% real LaTeX/titlesec accept — so a tailored CV using either doesn't silently
% corrupt (a plain 1-arg \renewcommand would typeset "*" or "[" as the title).
\makeatletter
...
% ---- Bullet list style (replaces enumitem). \cvitemsep/\cvtopsep/\cvparsep are
%      tunable lengths so the one-page shrink ladder can retighten list spacing. ----
...
runtime internals, Core Bluetooth, and cross-platform C++ interop. Track record of owning features
...
\begin{itemize}
  ...
  \item Built Swift network code for VPN/security layer (NetworkExtension), interoperating
        with existing C++ components
